// src/app/attendance/staff/page.jsx
"use client";
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 p-6 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading staff attendance page...</p>
          </div>
        </main>
        <Footer />
      </div>
...
function StaffAttendanceContent() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      if (data.success) {
        // Filter for staff role (role = 1)
        const allStaff = data.employees ? data.employees.filter(emp => emp.role === 1) : [];
        setStaff(allStaff);
      } else {
  ...
  const filteredStaff = staff.filter((emp, index, self) => 
  ...
  const toggleAllStaff = () => {
    if (selectedEmployees.size === filteredStaff.length) {
      // Deselect all
      setSelectedEmployees(new Set());
      setAttendanceData({});
    } else {
      // Select all
      const allIds = new Set(filteredStaff.map(emp => emp.id));
      const allData = {};
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">Mark attendance for all staff members at once</p>
              </div>
              <Link
                href="/attendance"
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between">
                ...
                >
                  {selectedEmployees.size === filteredStaff.length ? 'Deselect All' : 'Select All'}
                </button>
              </div>
            </div>
            ...
            <div className="divide-y divide-gray-200 max-h-96 overflow-y-auto">
              {filteredStaff.map((employee) => (
                <div key={`${employee.id}-${employee.station_id}`} className="p-4 hover:bg-gray-50 transition-colors">
          ...
          {selectedEmployees.size > 0 && (
            <div className="mt-6 flex justify-center">
              ...
              >
                {submitting ? 'Processing...' : `👤 Mark Attendance for ${selectedEmployees.size} Staff`}
              </button>
            </div>
          )}
        </main>
