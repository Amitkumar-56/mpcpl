// src/app/attendance/team-leaders/page.jsx
"use client";
          ...
          <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 lg:px-8 max-w-7xl">
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray-600">Loading team leader attendance page...</p>
              </div>
            </div>
          </div>
        </main>
...
function TeamLeaderAttendanceContent() {
  ...
  const fetchTeamLeaders = async () => {
    try {
      ...
      if (data.success) {
        // Filter for team leader role (role = 3)
        const allTeamLeaders = data.employees ? data.employees.filter(emp => emp.role === 3) : [];
        setTeamLeaders(allTeamLeaders);
      } else {
        setError(data.error || "Failed to fetch team leaders");
      }
    } catch (error) {
      console.error('Error fetching team leaders:', error);
      setError("Failed to load team leader data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredTeamLeaders = teamLeaders.filter((tl, index, self) => 
    (tl.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    tl.emp_code.toLowerCase().includes(searchTerm.toLowerCase())) &&
    // Remove duplicates based on employee ID
    self.findIndex(t => t.id === tl.id) === index
  );
  ...
  const toggleAllEmployees = () => {
    const availableEmployees = filteredTeamLeaders.filter(emp => !selectedEmployees.has(emp.id));
  ...
  const handleSubmitBulkAttendance = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSuccess(`Attendance processed successfully! ${data.summary.successful} team leaders marked, ${data.summary.failed} failed.`);
        setSelectedEmployees(new Set());
        setAttendanceData({});
        fetchTeamLeaders();
      } else {
          ...
          <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 lg:px-8 max-w-7xl">
            ...
            <div className="mb-4 sm:mb-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-600 mt-1 text-xs sm:text-sm lg:text-base">
                  Mark attendance for all team leaders
                </p>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <div className="bg-green-100 text-green-800 px-4 py-2 rounded-lg font-medium text-center">
                    {selectedEmployees.size} / {filteredTeamLeaders.length}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                ...
                >
                  {filteredTeamLeaders.filter(tl => !selectedEmployees.has(tl.id)).length === 0 ? 'Deselect All' : 'Select All'}
                </button>
            ...
            {loading ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <div className="text-center">
                  ...
                  <p className="mt-4 text-gray-600">Loading team leaders...</p>
                </div>
              </div>
            ) : filteredTeamLeaders.length === 0 ? (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
                <div className="text-gray-500">
                  <p className="text-lg">No team leaders found</p>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-200 max-h-96 overflow-y-auto">
                  {filteredTeamLeaders.map((teamLeader) => (
                    <div key={`${teamLeader.id}-${teamLeader.station_id}`} className="p-4 hover:bg-green-50 transition-colors">
                      <div className="flex items-center gap-4">
                        <input
                          type="checkbox"
                          checked={selectedEmployees.has(teamLeader.id)}
                          onChange={() => toggleEmployeeSelection(teamLeader.id)}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-medium text-gray-900">{teamLeader.name}</div>
                              <div className="text-sm text-gray-500">
                                {teamLeader.emp_code} • {teamLeader.station_name}
                              </div>
                            </div>
                          </div>
                        </div>

                        {selectedEmployees.has(teamLeader.id) && (
                          <div className="flex items-center gap-2">
                            <select
                              value={attendanceData[teamLeader.id]?.status || "Present"}
                              onChange={(e) => updateAttendanceData(teamLeader.id, 'status', e.target.value)}
                            ...
                            <input
                              type="time"
                              value={attendanceData[teamLeader.id]?.check_in_time || ""}
                              onChange={(e) => updateAttendanceData(teamLeader.id, 'check_in_time', e.target.value)}
                            ...
                            <input
                              type="time"
                              value={attendanceData[teamLeader.id]?.check_out_time || ""}
                              onChange={(e) => updateAttendanceData(teamLeader.id, 'check_out_time', e.target.value)}
                              className="px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                              placeholder="Out"
                            />
                            ...
                            <input
                              type="text"
                              value={attendanceData[teamLeader.id]?.remarks || ""}
                              onChange={(e) => updateAttendanceData(teamLeader.id, 'remarks', e.target.value)}
                              className="px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-green-500 w-32"
                              placeholder="Remarks"
                            />
                          </div>
                        )}
            ...
            {selectedEmployees.size > 0 && (
              <div className="mt-6 flex justify-center">
                ...
                >
                  {submitting ? 'Processing...' : `👥 Mark Attendance for ${selectedEmployees.size} Team Leaders`}
                </button>
              </div>
            )}
          </div>
