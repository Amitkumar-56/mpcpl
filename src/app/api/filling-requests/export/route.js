  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        fr.qty as "Quantity",
        fr.status as "Status",
        CASE WHEN fr.created IS NOT NULL THEN DATE_FORMAT(CONVERT_TZ(fr.created, '+00:00', '+05:30'), '%d/%m/%Y %h:%i %p') ELSE NULL END as "Created Date",
        CASE WHEN fr.completed_date IS NOT NULL THEN DATE_FORMAT(CONVERT_TZ(fr.completed_date, '+00:00', '+05:30'), '%d/%m/%Y %h:%i %p') ELSE NULL END as "Completed Date",
