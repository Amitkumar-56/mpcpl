...
export async function GET(request) {
  try {
    ...
    const customerQuery = `
      SELECT c.*, cb.hold_balance, cb.cst_limit, cb.amtlimit, cb.validity_days, cb.limit_expiry, cb.last_reset_date
      FROM customers c 
      LEFT JOIN customer_balances cb ON c.id = cb.com_id 
      WHERE c.id = ?
    `;
    ...
    const responseData = {
      customer: {
        ...
        credit_days: customer[0].credit_days, // Credit days from customers table
        payment_type:
          customer[0].billing_type == 1
  ...
  try {
    ...
    switch (action) {
      case "update_customer_profile":
        const { name, phone, email, address, gst_name, gst_number, status } =
          data;
