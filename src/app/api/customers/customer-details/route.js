...
async function checkAndHandleOverdueCustomer(customerId) {
  try {
    ...
    const creditDaysValue =
      parseInt(customer.credit_days) > 0 ? parseInt(customer.credit_days) : 7;

    console.log(
      "Using credit days from customer_balances.day_limit:",
      creditDaysValue
    );
...
export async function GET(request) {
  try {
    ...
    const blockLocationIds =
      customer[0].blocklocation
        ?.split(",")
...
async function handleUpdateProfile(customerId, data) {
  ...
  const updateProfileQuery = `UPDATE customers SET ${updateFields.join(
    ", "
  )} WHERE id = ?`;
...
async function handleUpdateDayLimits(customerId, data) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to update day limits" },
      { status: 500 }
    );
  }
}
...
async function handleCreateFillingRequest(customerId, data) {
  ...
  try {
    ...
    if (
      eligibility.dailyLimit > 0 &&
      eligibility.dailyUsed + requestedAmount > eligibility.dailyLimit
    ) {
      return NextResponse.json(
        {
    ...
    return NextResponse.json(
      { error: "Failed to create filling request" },
      { status: 500 }
    );
  }
}
...
async function handleUpdateRequestStatus(data) {
  ...
  try {
    ...
    if (requests.length === 0) {
      return NextResponse.json(
        { error: "Filling request not found" },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json({
      message: `Filling request ${status} successfully`,
    });
  } catch (error) {
...
async function handleProcessPayment(customerId, data) {
  ...
  console.log(
    `Processing payment of ${paymentAmount} for customer ${customerId}`
  );
