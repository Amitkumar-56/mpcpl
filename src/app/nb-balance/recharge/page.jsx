  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-4 text-gray-600">Loading Recharge Form...</p>
      </div>
    </div>
  );
}
...
function RechargeContent() {
  ...
  const validateForm = () => {
    ...
    if (!form.title?.trim()) {
      newErrors.title = "Recharge title is required";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!confirm("Are you sure you want to add this cash recharge?")) return;

    setLoading(true);
    try {
      const res = await fetch("/api/nb-balance/recharge", {
        method: "POST",
      ...
      if (data.success) {
        alert("Cash recharge added successfully!");
        router.push("/nb-balance/cash-management");
      } else {
        throw new Error(data.message || "Error adding recharge");
      }
    } catch (err) {
    ...
    return (
      <div className="flex h-screen bg-gray-100 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <Header />
          <div className="flex-1 overflow-y-auto bg-gradient-to-br from-green-50 to-emerald-100 py-8 px-4 sm:px-6 lg:px-8">
  ...
  return (
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <Header />
        <main className="flex-1 overflow-y-auto bg-gradient-to-br from-green-50 to-emerald-100 py-8 px-4 sm:px-6 lg:px-8">
          <div className="max-w-md mx-auto">
            {/* Balance Card */}
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 border border-green-200">
              <div className="text-center">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
              <div className="bg-gradient-to-r from-green-600 to-emerald-700 px-6 py-4">
                <h2 className="text-2xl font-bold text-white">Add Cash Recharge</h2>
                <p className="text-green-100 text-sm mt-1">Add money to non-billing account</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Recharge Title <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    name="title"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="e.g., Bank, Cash, Customer Payment"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                  />
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 bg-gradient-to-r from-green-600 to-emerald-700 text-white py-3 px-4 rounded-lg font-semibold shadow-md hover:from-green-700 hover:to-emerald-800 transition-all disabled:opacity-50"
                  >
                    {loading ? "Processing..." : "Add Recharge"}
                  </button>
