...
export default function CustomerDetailsClient() {
  ...
  const fetchCustomerDetails = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(
          data.error || `Failed to fetch customer details: ${res.status}`
        );
      }
                    ...
                    <p className="text-gray-600">
                      ...
                      <span className="font-semibold capitalize">
                        ...
                        customer.credit_days
                          ? `Credit Days (${customer.credit_days} days)`
                      ...
                      <p className="text-gray-600">
                        ...
                        <span
                          className={`font-semibold ${
                            new Date(customer.limit_expiry) < new Date()
                              ? "text-red-600"
                              : "text-green-600"
                          }`}
                        >
                          {new Date(customer.limit_expiry).toLocaleDateString(
                            "en-IN"
                          )}
                        </span>
                      </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                  ...
                  >
                    ...
                    >
                      ₹
                      {(
                        (customer.amtlimit || 0) - (customer.hold_balance || 0)
                      ).toLocaleString("en-IN")}
                    </div>
                  </div>
                </div>
...
function DetailsTab({ customer }) {
  const details = [
    ...
    { label: "Products", value: customer.productNames?.join(", ") || "N/A" },
    {
      ...
      value:
        customer.blockLocations?.length > 0
          ? customer.blockLocations.join(", ")
          : "No block locations",
    },
    {
      label: "Limit Expiry Date",
      value: customer.limit_expiry
