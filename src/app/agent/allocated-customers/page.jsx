...
export default function AllocatedCustomersPage() {
  ...
  useEffect(() => {
    ...
    if (agent?.id) {
      ...
      fetch(`/api/agent/commission-history?agentId=${agent.id}`)
        .then((res) => res.json())
        .then((json) => {
          setData(json?.allocatedCustomers || []);
        })
        ...
        <main className="flex-1 p-4 lg:p-6 overflow-auto">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-2xl font-bold text-gray-900">My Allocated Customers & Rates</h1>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Commission Rate (₹/L)</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {pageData.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-medium">₹{parseFloat(item.commission_rate || 0).toFixed(2)}</td>
                        </tr>
                      ))}
                      {pageData.length === 0 && (
                        <tr>
