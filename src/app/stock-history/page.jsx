...
function StockHistoryContent() {
  ...
  // ✅ हटाए गए फंक्शन - आपके पास stock डेटा नहीं है
  /*
  const handleViewDetails = (id) => {
    console.log('View details for ID:', id);
    console.log('Available stock data:', stock);
    console.log('Stock item found:', stock.find(item => item.id === id));
    
    router.push(`/stock/supply-details/${id}`);
  };
  ...
  const handleSendMessage = (id) => {
    ...
    const message = `Stock Details:\n` +
      `ID: ${stockItem.id}\n` +
      `Invoice: ${stockItem.invoice_number}\n` +
      `Product: ${stockItem.product_name}\n` +
      `Station: ${stockItem.station_name}\n` +
      `Quantity: ${stockItem.ltr} L\n` +
      `Status: ${stockItem.status}\n` +
      `Date: ${new Date(stockItem.created_at).toLocaleDateString('en-IN')}`;
    
    navigator.clipboard.writeText(message).then(() => {
      alert('Stock details copied to clipboard! You can now send this message.');
    }).catch(err => {
      console.error('Failed to copy:', err);
      alert('Failed to copy stock details');
    });
  };
  */
                          ...
                          data.rows.map((row) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* ✅ एक्शन सेल हटा दिया गया */}
                              </tr>
                            );
                          })
                        ) : (
                          <tr>
                            <td colSpan="10" className="px-6 py-12 text-center"> {/* ✅ colSpan 10 हो गया */}
