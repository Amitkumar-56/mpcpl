...
function TankerLogsContent() {
  ...
  const [tankerInfo, setTankerInfo] = useState(null);
  ...
  const fetchTankerInfo = async () => {
    try {
      ...
      if (result.success) {
        setTankerInfo(result.data.tanker);
      }
    } catch (error) {
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/tanker-audit-logs?tanker_id=${tankerId}`);
      const result = await response.json();
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Tanker ID is required</p>
          <button
            onClick={() => router.back()}
              ...
              <div>
                ...
                {tankerInfo && (
                  <p className="text-sm text-gray-600 mt-1">
                    Tanker ID: {tankerId} | Licence Plate: {tankerInfo.licence_plate || 'N/A'}
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">No activity logs available for this tanker.</p>
            </div>
          ) : (
...
export default function TankerLogs() {
  return (
    <Suspense 
      fallback={
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
