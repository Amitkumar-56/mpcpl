        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {['User', 'Action', 'Transfer ID', 'Details', 'Date', 'Time'].map((header) => (
                <th key={header} className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">
                  {header}
                </th>
              ))}
            </tr>
...
function StockTransfersActivityLogsContent() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      params.append('page', 'Stock Transfers');
      params.append('record_type', 'stock_transfer');
  ...
  const filtered = logs.filter(log => {
    const matchSearch = !search ||
      ...
      (log.unique_code && log.unique_code.toLowerCase().includes(search.toLowerCase()));
  ...
  async function exportToPDF(logs, actionFilter) {
    ...
    doc.text('Stock Transfers Activity Logs', 14, 14);
    ...
    // ── Stats summary row ──
    const statLabels = ['Total Transfers', "Today's Transfers", 'Active Users', 'Recent Updates'];
    ...
    doc.autoTable({
      startY: 55,
      head: [['#', 'User', 'Action', 'Transfer ID', 'Details', 'Date', 'Time']],
    ...
    doc.save(`stock-transfers-activity-logs-${Date.now()}.pdf`);
  }
  ...
  const BADGE_STYLES = {
    ...
    transfer: 'bg-purple-50 text-purple-700 ring-1 ring-purple-200',
  };
  ...
  const STATS = [
    { label: 'Total Transfers', value: total.toString(), trend: '+12% vs last week', color: 'blue' },
    { label: "Today's Transfers", value: todayCount.toString(), trend: 'Updated in real-time', color: 'green' },
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {/* Back Button - Navigates to /stock-transfers */}
              <button
              ...
              >
                <BackIcon />
                <span className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Back to Transfers</span>
              </button>
              
              <div className="h-8 w-px bg-gray-200" />
              
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-purple-700 flex items-center justify-center text-white shadow-md">
                  <TransferIcon />
                </div>
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Stock Transfer Activity Logs</h1>
                  <p className="text-sm text-gray-500 mt-0.5">Track and monitor all stock transfer activities</p>
                </div>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-4 border-b border-gray-100 bg-gray-50/40">
                <div className="flex items-center gap-2">
                  <LogsIcon />
                  <span className="text-sm font-semibold text-gray-700">Transfer Activity Timeline</span>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    ...
                    {availableActions.length > 0 ? (
                      ...
                      ['create', 'add', 'update', 'edit', 'delete', 'approve', 'reject', 'transfer'].map(action => (
                        <option key={action} value={action}>
                          {action.charAt(0).toUpperCase() + action.slice(1)}
                        </option>
                      ))
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      ...
                      <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">Transfer ID</th>
                    ...
                    {paginated.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center py-16">
                          <div className="text-gray-400">No transfer activity records found</div>
                        </td>
                      </tr>
                    ) : (
                      paginated.map((log, idx) => (
                        <tr key={log.id || idx} className="hover:bg-gray-50/80 transition-colors duration-150">
                          ...
                          <td className="px-6 py-3">
                            <span className="text-sm text-gray-700 font-mono">{log.unique_code || 'N/A'}</span>
                          </td>
                          <td className="px-6 py-3">
...
export default function StockTransfersActivityLogsPage() {
  return (
    <Suspense fallback={<LogsLoadingSkeleton />}>
      <StockTransfersActivityLogsContent />
    </Suspense>
  );
}