...
function VoucherWalletDriverEmpContent() {
  ...
  const fetchRentalTrips = async () => {
    try {
      console.log('Fetching rental trips...');
      const res = await fetch("/api/rental/trips?status=Open");
      if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
      const data = await res.json();
      console.log('Rental trips data:', data);
      setRentalTrips(data?.data || data || []);
    } catch (err) {
      console.error("Error fetching rental trips:", err);
      setRentalTrips([]); // Set empty array on error
    }
  };
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl p-5 w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleAddAdvance} className="space-y-4">
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={closeModal} className="flex-1 px-3 py-2.5 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50">Cancel</button>
                <button type="submit" name="add_advance" className="flex-1 px-3 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Add Advance</button>
              </div>
            </form>
          </div>
        </div>
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
          <div className="text-center max-w-md">
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              <div className="text-red-600 text-lg font-semibold mb-2">Application Error</div>
              <p className="text-red-700 mb-2">A client-side exception has occurred.</p>
              <p className="text-red-600 text-sm mb-4">{this.state.error?.message || 'Unknown error'}</p>
              <div className="flex gap-2 justify-center">
                <button onClick={() => window.location.reload()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm">Reload Page</button>
                <button onClick={() => window.history.back()} className="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded text-sm">Go Back</button>
              </div>
            </div>
          </div>
        </div>
